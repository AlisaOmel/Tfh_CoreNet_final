...
CalcCliffDelta <- function(sig_z, y, comb, sig_idx = NULL){
  ...
  # loop through each column in the z matrix (each latent factors)
...
CliffDeltaPermute_2 <- function(z, nrep, SLIDE_cd, comb, sig_idx){
  SLIDE_pvals = c()
  null_cds = data.frame(matrix(ncol = 4))
  for (rep in 1:nrep){
    print(rep)
  ...
  null_cds = null_cds[-1, ]
  return(list(SLIDE_pvals, null_cds))
}


GetPairwiseComb <- function(y) {
  vec <- sort(c(unique(y)), decreasing = FALSE)
  if (length(unique(y)) == 2){
    comb = list(c(unique(y)[[1]], unique(y)[[2]]))
  } else {
    combinations <- combn(vec, 2)
    comb <- vector("list", ncol(combinations))
    for (i in 1:ncol(combinations)){
      comb[[i]] <- c(combinations[, i])
    }
  }
  return(comb)
}
